...
Dependencies: Only stdlib (json, re, pathlib)
"""
...
class ArticulationMatchTool:
    ...
    def run(self, smc_courses: List[str], target_major: str) -> Dict[str, Any]:
        ...
        file_path = _DATA_ROOT / f"{slug}.json"
        
        if not file_path.exists():
            raise MajorNotFoundError(f"No articulation data found for major '{target_major}' (slug: {slug})")
