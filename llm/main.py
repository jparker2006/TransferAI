...
from llm.prompt_builder import build_prompt, PromptType, VerbosityLevel, PromptBuilder
...
class TransferAIEngine:
    ...
    def __init__(
        ...
        """
        Initialize the TransferAI engine.
        
        Args:
            documents: Pre-loaded documents (optional)
            llm: Language model instance to use for responses
            version: Version of TransferAI to use
            verbosity: Level of detail in responses (MINIMAL, STANDARD, DETAILED)
            debug_mode: Whether to include debug information in prompts
        """
        ...
        self._prompt_builder = PromptBuilder(verbosity=verbosity)
        logger.info(f"TransferAI Engine v{version} initialized with verbosity {verbosity}")

    def set_verbosity(self, verbosity: Union[VerbosityLevel, str]) -> None:
        """
        Update the verbosity level for responses.
        
        Args:
            verbosity: New verbosity level (MINIMAL, STANDARD, DETAILED)
        """
        self._prompt_builder.set_verbosity(verbosity)
        logger.info(f"Verbosity level updated to {verbosity}")
        
    def configure(self, **kwargs) -> None:
        """
        Configure the TransferAI engine.
        
        Args:
            **kwargs: Configuration options to override defaults.
                      Supported options: verbosity, debug_mode, timeout, temperature, max_tokens
        """
        ...
        # Handle verbosity specifically to ensure it's a VerbosityLevel enum
        if "verbosity" in kwargs and not isinstance(self.config["verbosity"], VerbosityLevel):
            # Convert string to enum if needed
            if isinstance(self.config["verbosity"], str):
                try:
                    self.config["verbosity"] = VerbosityLevel[self.config["verbosity"].upper()]
                except KeyError:
                    print(f"Invalid verbosity level: {self.config['verbosity']}. Using STANDARD.")
                    self.config["verbosity"] = VerbosityLevel.STANDARD
                    
        print(f"Engine configured with verbosity: {self.config['verbosity'].name}")
    ...
    def handle_query(self, query: str) -> Optional[str]:
        ...
        try:
            ...
            return self.format_multi_uc_response(filters["uc_course"], responses_by_uc.values())
