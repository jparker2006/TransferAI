...
test_prompts_v12_eval = [
    # ✅ R10 Fix: Multi-UC articulation
    "If I complete CSE 8A and 8B, is that one full path?",       # test #6 (index 6)
    "Which courses satisfy MATH 20A and 20B?",                   # test #21 (index 20)

    # ✅ R11.2 Fix: Reverse CCC match logic
    "If I took CIS 36A, can it satisfy more than one UCSD course?",  # test #31 (index 30)

    # 🟡 Not yet fixed: Combo CCC articulation (R13)
    "Can I satisfy Group 3 with CHEM 6A and PHYS 2A?",           # test #27 (index 26)

    # ✅ Regression check (baseline UC single course match)
    "Which De Anza courses satisfy CSE 8A at UCSD?",             # test #1 (index 0)
    "Which courses count for CSE 12 at UCSD?"                    # test #10 (index 9)
]
...
def run_test_suite():
    ...
    for i, prompt in enumerate(test_prompts_v12_eval, 1):
